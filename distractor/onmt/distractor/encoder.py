...
class DistractorEncoder(nn.Module):
    ...
    def __init__(self, enc_topic_embeddings,rnn_type,
        ...
        self.l_ques = l_ques
        self.l_ans = l_ans
        self.topic_emb = enc_topic_embeddings
        self.cq_att = CQAttention(d_model=hidden_size)
        self.PAD = pad
        self.cq_resizer = Initialized_Conv1d(hidden_size * 4, hidden_size)
        self.dropout = dropout
        # self.topic_emb = Btm(src_dict).prob_emb
        ...
        self.norm_linear_topic = nn.Linear(330, 300)
    ...
    def forward(self, src, ques, ans, sent_length, word_length, ques_length, ans_length):
        ...
        sorted_ans_emb = self.norm_linear_topic(sorted_ans_emb)
        ...
        #sorted_ans_bank = sorted_ans_bank.mul(sorted_topic_ans)
        ...
        sorted_word_emb = self.norm_linear_topic(sorted_word_emb)
       ...
       # sorted_word_bank = sorted_word_bank.mul(sorted_topic_src)
        ...
        sorted_ques_emb = self.norm_linear_topic(sorted_ques_emb)
       # sorted_topic_ques = torch.tensor(sorted_topic_ques, dtype=torch.float32).to(self.device)
        ...
        # sorted_ques_bank = sorted_ques_bank.mul(sorted_topic_ques)
        ...
        sorted_quesinit_emb = self.norm_linear_topic(sorted_quesinit_emb)

        sorted_quesinit_bank, sorted_quesinit_state, _ = self.init_encoder(sorted_quesinit_emb, sorted_quesinit_length)  # encode

        # sorted_quesinit_bank = sorted_quesinit_bank.mul(sorted_topic_ques)
        quesinit_bank, quesinit_state = wrapped_quesinit.remap(sorted_quesinit_bank, sorted_quesinit_state)  # remap


        # co-attention
        # maskS = (torch.ones_like(src.view(src.size()[0], src.size()[1] * src.size()[2])) *
        #          self.PAD != src).float()
        # maskQ = (torch.ones_like(ques) *
        #          self.PAD != ques).float()
        X = self.cq_att(sent_bank, ques_bank)#, maskS, maskQ)
        G = self.cq_resizer(X)
        g = F.dropout(G, p=self.dropout, training=self.training)
        G = G.permute(2, 0, 1)
        g = g.permute(2, 0, 1)

        oneG = (torch.ones_like(g)).float()
        # final representation of sentence-level hidden states
        sent_context = g.mul(sent_bank)+(oneG-g).mul(G)
        # static attention
        ...
        return word_bank, sent_bank,sent_context,ques_bank, quesinit_state, static_attn
